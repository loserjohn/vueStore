	...
	<view>
		<scroll-view class="page" scroll-y="true" @scrolltolower="loadMore">
			<view v-if="lists.length > 0">
				<block v-for="(item, index) in lists" :key="index">
					<view class="proItemsBox ">
						...
						<view class=" tui-skeleton-fillet flex flex_center " style="height: 80rpx;" @click="_showDetail(item)">
							<view class="f1"></view>
							<view class="cm_des">共 {{ item.goods_num }} 件商品</view>
							<view style="margin-left: 24rpx;">实付：<text class=" cm_t_32 cm_prize">￥{{ item.pay_price }}</text></view>
			...
			<view class="flex flex_center" style="width: 100%;height: 90vh;" v-else>
				<tui-no-data :fixed="false" imgUrl="/static/img/toast/img_noorder_3x.png">暂无订单</tui-no-data>
			</view>
