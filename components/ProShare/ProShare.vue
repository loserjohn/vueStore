...
<script>
	...
	export default {
		...
		watch:{
			good(val,o){
				if(val.goods_code){
					this.imgSrc = this.good.goods_main_img;
					// this.imgSrc = '../../../static/img/fx_ywy.jpg';
					this.Title = this.good.goods_title;
					this.PriceTxt = this.good.goods_price;
					this.ShortTitle = this.good.goods_short_title;
					this.shareUrl = `${SET.mainUrl}/store/index.html#/pages/main/details/details?code=${this.good.goods_code}`;
					// console.log(5555,this.shareUrl  )	 
				}
			}
		},
		methods: {
			...
			_share(tapIndex) {
				...
				uni.share({
					...
					type: 2,
					title: '睿举',
					summary: '邀您下载睿举，让您淘宝天猫购物省钱',
					imageUrl: that.poster.finalPath,
					href: that.shareUrl,
					success: res => {
						//console.log('success:' + JSON.stringify(res));
						plus.nativeUI.toast('分享成功');
						// that.errMsg = '温馨提示：分享失败，请及时检查微信及酷熊优购pro相机等权限是否打开，必要时可以卸载重装app，微信重装前切记先做好数据备份，以防丢失，具体可点击右下角问号图标进入新手引导'
						// that.$refs.resultModal._showDialog();
						clipboard.setText('');
					},
					fail: e => {
						// plus.nativeUI.closeWaiting()
						that.errMsg =
							'温馨提示：分享失败，请及时检查微信及酷熊优购pro相机等权限是否打开，必要时可以卸载重装app，微信重装前切记先做好数据备份，以防丢失，具体可点击右下角问号图标进入新手引导';
						that.$refs.resultModal._showDialog();
						clipboard.setText('');
					}
				});
				...
				try {
					const d = await getSharePoster({
						...
						drawArray: ({
							...
							const lineHeight = bgObj.width * 0.07;
							...
							return new Promise((rs, rj) => {
								rs([
									...
									},
									{
										...
										dx: bgObj.width*.7 - dx ,
										dy: bgObj.height - dx - bgObj.width * 0.3,
									},
									{
										type: 'text',
										// fontStyle: 'italic',
									...
									
									{
										...
										allInfoCallback({
											...
											let h =  obj.lineFeed.lineHeight *  obj.lineFeed.lineNum + obj.dy
											console.log(111,h)
											//也可以return promise对象
											return new Promise((rs, rj) => {
												setTimeout(() => {
									...
									},
									{
										...
										allInfoCallback({
											...
											const obj = drawArray.find(item => item.id === 'tag2');
